...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Optional route-level session coalescing. A coalesced route keeps one
        # session open across a burst of deliveries, queues deliveries that
        # arrive while the agent is busy, and closes the session only after the
        # configured quiet period. State is intentionally process-local: after
        # a gateway restart the next delivery starts a fresh incident session.
        self._coalesced_sessions: Dict[str, dict] = {}
        self._coalesced_expiry_tasks: Dict[str, asyncio.Task] = {}
        self._coalesced_routes_by_chat_id: Dict[str, str] = {}
        # Close-turn responses (including interim/error sends) are persisted in
        # the transcript but never delivered to the configured destination.
        self._silenced_session_chat_ids: set[str] = set()
    ...
    async def disconnect(self) -> None:
        expiry_tasks = list(self._coalesced_expiry_tasks.values())
        for task in expiry_tasks:
            if not task.done():
                task.cancel()
        if expiry_tasks:
            await asyncio.gather(*expiry_tasks, return_exceptions=True)
        self._coalesced_expiry_tasks.clear()

        # A graceful gateway restart must not strand open coalesced rows with
        # ended_at=NULL. Active adapter tasks have already been drained/cancelled
        # by the runner before disconnect; close every remaining incident via the
        # normal SessionDB API before discarding process-local routing state.
        for state in list(self._coalesced_sessions.values()):
            event = state.get("event")
            chat_id = state.get("chat_id")
            if event is not None and chat_id:
                await self._end_webhook_session(
                    event,
                    str(chat_id),
                    end_reason="webhook_shutdown",
                )
        self._coalesced_sessions.clear()
        self._coalesced_routes_by_chat_id.clear()
        self._silenced_session_chat_ids.clear()
        if self._runner:
            await self._runner.cleanup()
            self._runner = None
    ...
    async def send(
        ...
        """
        if chat_id in self._silenced_session_chat_ids:
            logger.info(
                "[webhook] Suppressed close-turn delivery for %s", chat_id
            )
    ...
    def _contains_template_value(value: Any) -> bool:
        """Whether a nested delivery config contains payload placeholders."""
        if isinstance(value, str):
            return bool(re.search(r"\{[^{}]+\}", value))
        if isinstance(value, dict):
            return any(
                WebhookAdapter._contains_template_value(item)
                for item in value.values()
            )
        if isinstance(value, (list, tuple)):
            return any(
                WebhookAdapter._contains_template_value(item) for item in value
            )
        return False

    @staticmethod
    def _coalesced_idle_timeout(route_config: dict) -> Optional[float]:
        """Return the quiet-period timeout for a safely coalescible route."""
        if route_config.get("session_mode") != "coalesced":
            return None
        # A shared chat_id cannot safely carry payload-templated destinations:
        # one delivery could overwrite where another run's response is sent.
        if WebhookAdapter._contains_template_value(
            route_config.get("deliver_extra", {})
        ):
            return None
        raw = route_config.get("session_idle_timeout_seconds", 1800)
        try:
            timeout = float(raw)
        ...
        if not math.isfinite(timeout):
            timeout = 1800.0
        # Bound accidental configs while keeping a small testable minimum.
        return min(604800.0, max(1.0, timeout))

    def _chat_id_for_delivery(
        self,
        route_name: str,
        delivery_id: str,
        route_config: dict,
        ...
        """Return a per-delivery or current coalesced session chat id."""
        timeout = self._coalesced_idle_timeout(route_config)
        if timeout is None:
            return f"webhook:{route_name}:{delivery_id}"

        state = self._coalesced_sessions.get(route_name)
        if state is None:
            state = {
                "chat_id": f"webhook:{route_name}:coalesced:{delivery_id}",
                "timeout": timeout,
                "last_delivery_at": now,
                "close_prompt": route_config.get("session_close_prompt"),
            }
            self._coalesced_sessions[route_name] = state
        else:
            state["timeout"] = timeout
            state["last_delivery_at"] = now
            state["close_prompt"] = route_config.get("session_close_prompt")

        # A new delivery makes the current incident live again. Completion of
        # the final queued turn schedules a fresh full quiet period.
        expiry = self._coalesced_expiry_tasks.pop(route_name, None)
        if expiry is not None and not expiry.done():
            expiry.cancel()
        chat_id = str(state["chat_id"])
        self._coalesced_routes_by_chat_id[chat_id] = route_name
        return chat_id

    def _schedule_coalesced_expiry(
        self,
        route_name: str,
        event: "MessageEvent",
        session_chat_id: str,
    ) -> None:
        """Close a coalesced session after its route has stayed quiet."""
        state = self._coalesced_sessions.get(route_name)
        if state is None or state.get("chat_id") != session_chat_id:
            return

        old_task = self._coalesced_expiry_tasks.pop(route_name, None)
        if old_task is not None and not old_task.done():
            old_task.cancel()

        timeout = float(state.get("timeout", 1800.0))
        task = asyncio.create_task(
            self._expire_coalesced_session(
                route_name,
                event,
                session_chat_id,
                timeout,
        ...
        self._coalesced_expiry_tasks[route_name] = task

    async def _expire_coalesced_session(
        self,
        route_name: str,
        event: "MessageEvent",
        session_chat_id: str,
        ...
        """Idle timer worker for one coalesced webhook incident."""
        try:
            await asyncio.sleep(timeout)
            runner = self.gateway_runner
            key_fn = getattr(runner, "_session_key_for_source", None)
            session_key = key_fn(event.source) if callable(key_fn) else None

            # Never close work that is still active or waiting in the adapter's
            # queue. The final turn's completion hook will replace this timer;
            # this loop is a safety net for unusually long cleanup windows.
            while session_key and (
            ...
            # Establish the rollover boundary before any await: a delivery
            # arriving from this point onward starts a fresh incident instead
            # of reviving the session being closed.
            close_prompt = state.get("close_prompt")
            self._coalesced_sessions.pop(route_name, None)
            current = asyncio.current_task()
            if self._coalesced_expiry_tasks.get(route_name) is current:
                self._coalesced_expiry_tasks.pop(route_name, None)

            if isinstance(close_prompt, str) and close_prompt.strip():
                # Silence is enforced in send(), not entrusted to the model.
                # toolsets_for_source also narrows this turn to read-only
                # session_search, so untrusted alert history cannot mutate
                # files, skills, or memory during note synthesis.
                self._silenced_session_chat_ids.add(session_chat_id)
                close_event = MessageEvent(
                    text=close_prompt.strip(),
                    message_type=MessageType.TEXT,
                    source=event.source,
                    raw_message={
                        "event": "coalesced_session_close",
                        "route": route_name,
                    },
                    message_id=f"coalesced-close-{int(time.time() * 1000)}",
                    internal=True,
                    metadata={"webhook_coalesced_close": True},
                    allow_gateway_control=False,
                )
                await self.handle_message(close_event)
                # handle_message installs the guard synchronously before its
                # background task starts. If it did so, that task owns closure
                # through on_processing_complete; otherwise fall back below.
                if session_key and session_key in self._active_sessions:
                    return

            self._silenced_session_chat_ids.discard(session_chat_id)
            self._coalesced_routes_by_chat_id.pop(session_chat_id, None)
            await self._end_webhook_session(
                event,
                session_chat_id,
                end_reason="webhook_idle_timeout",
            )
        except asyncio.CancelledError:
            return
        except Exception:
            logger.exception(
                "[webhook] Failed expiring coalesced session %s for route %s",
                session_chat_id,
                route_name,
            )
            self._coalesced_sessions.pop(route_name, None)
            ...
            await self._end_webhook_session(
                event,
                session_chat_id,
                end_reason="webhook_idle_timeout_error",
            )
    ...
    def toolsets_for_source(self, source) -> Optional[List[str]]:
        ...
        if chat_id in self._silenced_session_chat_ids:
            # The close turn only synthesizes a candidate note into this
            # transcript. It cannot write files, memory, or skills.
            return ["session_search"]

        route_name = self._coalesced_routes_by_chat_id.get(chat_id)
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Queue coalesced webhook follow-ups without interrupt/ACK behavior."""
        if (event.metadata or {}).get("webhook_coalesced_delivery"):
            ...
            if session_key in self._active_sessions:
                ...
                logger.info(
                    "[webhook] Coalesced delivery queued for active session %s",
                    session_key,
                )
                return
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        # Per-delivery sessions remain the default. Routes that explicitly opt
        # into ``session_mode: coalesced`` reuse one key during an alert burst;
        # BasePlatformAdapter/GatewayRunner then serialize queued follow-ups.
        session_chat_id = self._chat_id_for_delivery(
            route_name,
            delivery_id,
            route_config,
            now,
        ...
        coalesced = self._coalesced_idle_timeout(route_config) is not None

        # Store delivery info for send(). A coalesced route is allowed only
        # with a non-templated target, and the first target stays pinned for the
        # entire incident so later deliveries cannot misroute an earlier reply.
        deliver_config = {
            "deliver": route_config.get("deliver", "log"),
            "deliver_extra": self._render_delivery_extra(
                route_config.get("deliver_extra", {}), payload
        ...
        if not coalesced or session_chat_id not in self._delivery_info:
        ...
        event = MessageEvent(
            ...
            # Coalesced deliveries use the adapter's dedicated pending slot
            # above, bypassing the gateway's interrupt/ACK/FIFO-cap path.
            metadata={"webhook_coalesced_delivery": True} if coalesced else {},
            allow_gateway_control=not coalesced,
        )
        if coalesced:
            state = self._coalesced_sessions.get(route_name)
            if state is not None and state.get("chat_id") == session_chat_id:
                state["event"] = event
    ...
    async def on_processing_complete(
        ...
        """Close one-shot runs or arm expiry for a coalesced route.

        Per-delivery webhook sessions are closed immediately as before.
        Coalesced sessions stay open after each turn and receive a fresh idle
        timer. When that timer fires it may enqueue one final, silent close
        prompt (for incident learning/notes), whose completion closes the row.
        This hook runs at the true end of success, failure, and cancellation
        paths, so neither mode leaks ghost sessions.
        """
        session_chat_id = event.source.chat_id
        if (event.metadata or {}).get("webhook_coalesced_close"):
            try:
                await self._end_webhook_session(
                    event,
                    session_chat_id,
        ...
        route_name = self._coalesced_routes_by_chat_id.get(session_chat_id, "")
        state = self._coalesced_sessions.get(route_name)
        if state is not None and state.get("chat_id") == session_chat_id:
            state["event"] = event
            self._schedule_coalesced_expiry(
                route_name,
                event,
                session_chat_id,
            )
